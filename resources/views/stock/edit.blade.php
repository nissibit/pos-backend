...
<div class="card">
    <div class="card-header">
        <i class="fa fa-edit"> Editar Stock <b>{{$stock->name }}</b></i>
    </div>
    ...
    <div class="card-body">
        <form class="form-horizontal" method="POST" action="{{ route('stock.update', $stock->id) }}">
            ...
            <div class="row input-group-sm">
                ...
                <input type="text" class="form-control" name="product_name" id="product_name" value="{{ old('product_name', $stock->product->name ?? '') }}"  readonly="readonly" />   
                @else
                ...
                <div class="form-group col-sm">
                    ...
                    <input type="text" class="form-control @error('product_id') is-invalid @enderror" name="name" id="name" value="{{ old('name', $stock->name ?? '') }}"  readonly="readonly" />                
                    @error('product_id')
                ...
                @endif
                <input type="hidden" class="form-control" name="product_id" id="product_id" value="{{ old('product_id', $stock->product_id ?? '') }}"  readonly="readonly" />                
            </div>
            <div class="row">
                <div class="col-sm-6 form-group input-group-sm">
                    ...
                    <select class="form-control @error('store_id') is-invalid @enderror" name="store_id">
                        ...
                        <option value="{{ $store->id }}" {{ old('store_id', $stock->store_id ?? '') == $store->id ? 'selected' : '' }}>  {{ $store->name }}</option>                    
                        @endforeach
                    </select>
                    @error('store_id')
                    <span class="invalid-feedback" role="alert">
