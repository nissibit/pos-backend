<div class="card">
    <?php
    $total = $moneyflow->store()->sum('quantity');
    $stores = $moneyflow->store()->latest()->paginate(10);
    ?>
    <div class="card-header">
        {{ "Total :#".$total }}
        <span>
            <a href="{{ route('stock.create', ['id' => $moneyflow->id]) }}" class="btn btn-outline-secondary">
    ...
    <div class="card-body">
        <table class="table table-bordered table-hover table-responsive-sm table-sm">
            ...
            <tbody>
                ...
                <?php
                $stock = App\Models\Stock::where('moneyflow_id', $moneyflow->id)->where('store_id', $store->id)->first();
                ?>
                <tr>
