...
<div class="card">
    <div class="card-header">
        <h2><i class="fa fa-folder-open"> Abrir Fundo </i></h2>        
    </div>
    <div class="card-body">
        <form role="form" autocomplete="off" class="form-horizontal" action="{{ route('fund.store') }}" method="post">
            {{ csrf_field() }}
            <div class="row">
                <div class="form-group col input-group-sm">
                    <label class="control-label">
                        Valor inicial (MT) <i class="text-danger">*</i>
                    </label>
                    <input type="text" class="form-control @error('initial') is-invalid @enderror" name="initial" id="initial" value="{{ old('initial', $fund->initial ?? '0') }}"  />
