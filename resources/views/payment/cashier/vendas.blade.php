<div class="alert alert-success">    
    <strong>Total das vendas visiveis: {{ number_format($payments->sum('topay') ?? 0, 2)}}</strong>
</div>
...
//        echo \Lang::get("messages.sidebar.{$route}")
        ...
        <tr>
            ...
            <td><a href="{{ route('payment.show', $payment->id) }}">{{ $payment->payment->id }}</a></td>
            ...
            <td><a href="{{ route('payment.show', $payment->id) }}">{{ $payment->payment->created_at->format('d-m-Y') ?? 'N/A' }}</a></td>                                
            <td>
