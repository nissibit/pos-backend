<div class="alert alert-success">    
    <strong>@lang('messages.cashier.total.credit'): {{ number_format($credits->sum('topay') ?? 0, 2)}} | #{{$credits->count()}}</strong>
</div>
<table class="table table-bordered table-hover table-responsive-sm table-sm">           
    ...
    <tbody>
        @foreach($credits ?? [] as $key => $payment)
        <tr>
            <td>{{$key+1}}</td>
            ...
            <td>{{$payment->payment->created_at->format("d/m/Y h:i")}}</td>
