...
<div class="card">
    <div class="card-header">
        <i class="fa fa-edit"> Editar Transference <b>{{$transference->name }}</b></i>
    </div>

    <div class="card-body">
        <form class="form-horizontal" method="POST" action="{{ route('transference.update', $transference->id) }}">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <div class="row input-group-sm">
                @if($product ?? null != null)
                <label for="name" class="control-label">Produto <b class="text-danger">*</b></label>
                <input type="text" class="form-control" name="product_name" id="product_name" value="{{ old('product_name', $transference->product->name ?? '') }}"  readonly="readonly" />   
                @else

                <div class="form-group col-sm">
                    <label for="name" class="control-label">Seleccionar Produto <b class="text-danger">*</b></label>
                    <select class="form-control @error('product_combo') is-invalid @enderror" id="product_combo" name="product_combo">
                ...
                <div class="form-group col-sm">
                    ...
                    <input type="text" class="form-control" name="barcode" id="barcode" value="{{ old('barcode', $transference->barcode ?? '') }}" />                
                </div>
                <div class="form-group col-sm">
                    <label for="othercode">Outro Codigo </label>
                    <input type="text" class="form-control" name="othercode" id="othercode" value="{{ old('othercode', $transference->othercode ?? '') }}" />                
                </div>
                <div class="form-group col-sm">
                    <label for="name">Produto </label>
                    <input type="text" class="form-control @error('product_id') is-invalid @enderror" name="name" id="name" value="{{ old('name', $transference->name ?? '') }}"  readonly="readonly" />                
                    @error('product_id')
                ...
                @endif
                <input type="hidden" class="form-control" name="product_id" id="product_id" value="{{ old('product_id', $transference->product_id ?? '') }}"  readonly="readonly" />                
            </div>
            <div class="row">
                <div class="col-sm-6 form-group input-group-sm">
                    ...
                    <select class="form-control @error('store_id') is-invalid @enderror" name="store_id">
                        ...
                        <option value="{{ $store->id }}" {{ old('store_id', $transference->store_id ?? '') == $store->id ? 'selected' : '' }}>  {{ $store->name }}</option>                    
                        @endforeach
                    </select>
                    @error('store_id')
                    <span class="invalid-feedback" role="alert">
                ...
                <div class="col-sm-6 form-group input-group-sm">
                    ...
                    <input id="quantity" type="quantity" class="form-control @error('quantity') is-invalid @enderror" name="quantity" value="{{ old('quantity', $transference->quantity ?? '') }}"  >
                    @error('quantity')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
