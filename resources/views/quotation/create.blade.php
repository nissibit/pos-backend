...
<div class="card">
    <form class="form-horizontal" method="POST" action="{{ route('quotation.store') }}">
        ...
        <div class="card-header">
            <div class="row">
                <div class="col">
                    <h3><i class="fa fa-book-open"> </i> Nova Cotação</h3>
                    <input type="hidden" name="store_id" id="store_id" value="{{ $store->id }}" />
                    <input type="hidden" class="datepicker" name="day" id="day" value="{{ old('day',$quotation->day ?? \Carbon\Carbon::today()->format('Y-m-d')) }}" readonly="readonly" />           
                </div>
