<div class="row">
    <div class="form-group col btn-group-sm">
        <a href="{{ route('server.create') }}" class="btn btn-outline-secondary">
            <i class="fas fa-plus-circle"> criar</i>
        </a> 
        <a href="{{ route('server.index') }}" class="btn btn-outline-secondary  ml-1">
            <i class="fas fa-list"> listar</i>
        </a>
    </div>
    
    <div class="form-group col input-group-sm">
        <form id="unity_search" role="form" autocomplete="off" action="{{ route('server.search') }}" method="post" class="m-sm-1">
            {{ csrf_field() }}
