<div class="card">
    <?php
    $total = $server->account()->count();
    $accounts = $server->account()->latest()->paginate(10);
    ?>
        ...
        <form  role="form" autocomplete="off" action="{{ route('account.store') }}" method="post" class="m-sm-1">
            ...
            <div class="row">
                <div class="col input-group-sm input-group">
                    ...
                    <input type="hidden" name="server_id" class="form-control @error('server_id') is-invalid @enderror" placeholder="cliente" required=""  value="{{ old('server_id', $server->id ?? '0') }}" />                        
                    @error('server_id')
